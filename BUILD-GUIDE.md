...
> The Risk Monitor must pull live data from the CORE sources in DATA-SOURCES.md — GDELT, a few RSS feeds including at least one German-language feed, and PEGELONLINE for Rhine water levels. Keep each source in its own small function. Use the LLM to classify each item as logistics-relevant or not, tag relevant ones with chokepoint / type / severity, and write them to `risk_state.json`. Skip everything marked OPTIONAL for now. It must also be able to load the injected Hamburg event in the same format.
